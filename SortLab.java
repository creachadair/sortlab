...
public class SortLab extends JApplet implements ChangeListener, ActionListener, ItemListener {
  public void init() {
    p_running = false;
    p_thread = null;
    p_algo = -1;
  ...
  public void paint(Graphics g) {
    if (p_running && p_done) ui_run.doClick();
  ...
  public void stateChanged(ChangeEvent e) {
    ...
    if (src == ui_speed) updateSpeed();
    else if (src == ui_size) updatePointSize();
  }
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (src == ui_algo) updateAlgorithm();
    else if (src == ui_stype) updateSortType();
    else if (src == ui_step) doSingleStep();
    else if (src == ui_run) {
      if (p_running) stopRunning();
      else startRunning();
  ...
  */
  ...
  private void setupUserInterface() {
    ...
    ui_step = new JButton("Step");
    ui_step.setFont(f_plain);
    ui_step.addActionListener(this);
    ui_step.setPreferredSize(new Dimension(60, 30));
    ui_dis.add(ui_step);

    ui_reset = new JButton("Reset");
    ui_reset.setFont(f_plain);
    ...
    ui_reseed = new JButton("Reseed");
    ui_reseed.setFont(f_plain);
    ...
    ui_algo = new JComboBox();
    ui_algo.setFont(f_plain);
    for (int ix = 0; ix < s_algs.length; ++ix) ui_algo.addItem(s_algs[ix].getAlgorithmName());
    ...
    ui_comps = new JLabel("?");
    ui_comps.setFont(f_plain);
    ui_moves = new JLabel("?");
    ui_moves.setFont(f_plain);
    ui_csize = new JLabel("?");
    ui_csize.setFont(f_plain);
    ui_nelts = new JLabel("?");
    ui_nelts.setFont(f_plain);
    ...
    lb = new JLabel("Algorithm:");
    lb.setFont(f_bold);
    feed.add(lb);
    feed.add(ui_algo);
    feed.add(Box.createHorizontalStrut(15));
    lb = new JLabel("Ordering:");
    lb.setFont(f_bold);
    feed.add(lb);
    feed.add(ui_stype);
    feed.add(Box.createHorizontalStrut(15));
    lb = new JLabel("Comparisons:");
    lb.setFont(f_bold);
    feed.add(lb);
    feed.add(ui_comps);
    feed.add(Box.createHorizontalStrut(15));
    ...
    CompoundBorder c =
        new CompoundBorder(new EmptyBorder(2, 2, 2, 2), LineBorder.createBlackLineBorder());
  ...
  private static final String[] speed_labels = {"Crawl", "Slow", "Medium", "Fast", "Hyper"};

  private static final int[] speed_values = {500, 250, 100, 20, 5}; /* delays, in milliseconds */
