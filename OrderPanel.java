...
public class OrderPanel extends JPanel implements CustomOrder, SortModel {
  /* Order selectors    */
  ...
  public int compareElements(int i, int j) {
    ...
    if (m_points[i] < m_points[j]) return O_LT;
    else if (m_points[i] == m_points[j]) return O_EQ;
    else return O_GT;
  }
  ...
  public int compareElementToValue(int i, int value) {
    ...
    if (m_points[i] < value) return O_LT;
    else if (m_points[i] == value) return O_EQ;
    else return O_GT;
  }
  ...
  public boolean isSorted() {
    for (int ix = 0; ix < m_points.length - 1; ++ix)
      if (m_points[ix] > m_points[ix + 1]) return false;
  ...
  public void orderElements(int[] array) {
    switch (m_order_type) {
      ...
      case ORDER_ASCENDING:
        for (int ix = 0; ix < array.length; ++ix) array[ix] = ix;
        break;
      case ORDER_DESCENDING:
        for (int ix = 0, jx = array.length - 1; ix < array.length; ++ix, --jx) array[ix] = jx;
        break;
    }
  }
  ...
  public void paint(Graphics g) {
    ...
    for (int ix = 0; ix < m_points.length; ++ix) {
      ...
      int vpos = (int) (height - pointHeight - m_points[ix] * pointHeight);
      ...
      if (show_feedback) {
        ...
        if (ix == m_lasts_1 || ix == m_lasts_2) {
          ...
          g.drawLine((int) (hpos + (pointWidth / 2)), 0, (int) (hpos + (pointWidth / 2)), height);
          ++halves;
        }
      }
      ...
      if (show_feedback) {
        if (ix == m_lastc_1 || ix == m_lastc_2) {
          ...
          g.fillRect(
              hpos,
              vpos,
              ((halves == 2) ? (int) (pointWidth / 2) : (int) pointWidth),
              (int) pointHeight);
        }
        if (ix == m_lasts_1 || ix == m_lasts_2) {
          ...
          g.fillRect(
              hpos + ((halves == 2) ? (int) (pointWidth / 2) : 0),
              vpos,
              ((halves == 2) ? (int) (pointWidth / 2) : (int) pointWidth),
              (int) pointHeight);
        }
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    System.out.println(
        "Sorting took " + op.getNumCompares() + " comparisons and " + op.getNumMoves() + " swaps.");
    if (op.isSorted()) System.out.println(op.getNumElements() + " elements were sorted.");
    else System.out.println("The sort failed.");
    Thread.sleep(1000);
