/*
  ...
  Info:     $Id: MergeSort.java,v 1.2 2006/02/28 06:11:54 sting Exp $
...
public class MergeSort extends Sort
{
	...
	for(int ix = 0; ix < n_elt; ++ix)
	    m_piles.addLast(new Range(ix, ix));
	...
	case S_IDLE: // Select more work, or mark sorting as complete
	    if(m_piles.size() == 1) 
		m_next.addLast(m_piles.removeFirst());
	...
	case S_MERGING: // Comparison/load phase of merge
	    ...
	    else if(m_lcur > m_left.m_hi)
		grab = m_rcur++;
	    else if(m_rcur > m_right.m_hi)
		grab = m_lcur++;
	    else if(getModel().compareElements(m_lcur, m_rcur) <= 
		    SortModel.O_EQ) 
		grab = m_lcur++;
	    else
		grab = m_rcur++;
	...
	case S_COPYING: // Copying phase of merge
	    ...
	    getModel().exchangeElement(m_left.m_lo + m_ccur,
				       m_temp[m_ccur]);
	    ++m_ccur;
	    break;
	...
	public int size() { return m_hi + 1 - m_lo; }
