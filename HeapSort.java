...
public class HeapSort extends Sort {
  ...
  public void nextStep() {
    if ((m_state & S_HEAPIFY) != 0) advanceHeapify();
    else if ((m_state & S_EXTRACT) != 0) advanceExtract();
  }
  ...
  private void advanceExtract() {
    ...
    if (lc < m_root && getModel().compareElements(lc, max) == SortModel.O_GT) {
      max = lc;
    }

    if (rc < m_root && getModel().compareElements(rc, max) == SortModel.O_GT) {
      max = rc;
    }
  ...
  private void advanceHeapify() {
    ...
    if (lc < n_elt && getModel().compareElements(lc, max) == SortModel.O_GT) {
      max = lc;
    }

    if (rc < n_elt && getModel().compareElements(rc, max) == SortModel.O_GT) {
      max = rc;
    }
