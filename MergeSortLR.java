...
public class MergeSortLR extends Sort {
  ...
  public void nextStep() {
    if (m_to_do.empty()) return;

    Range r = (Range) m_to_do.peek();

    switch (r.getAction()) {
      case A_SORT:
        if (r.isUnit()) m_to_do.pop(); /* Singletons are already sorted */
        else {
          ...
          if (!right.isUnit()) m_to_do.push(right);

          if (!left.isUnit()) m_to_do.push(left);
        }
        break;
      ...
      case A_MERGE:
        ...
        if (m_lo_cursor < m_midmark
            && (m_hi_cursor > r.getHi()
                || getModel().compareElements(m_lo_cursor, m_hi_cursor) <= SortModel.O_EQ)) {

          m_merge[m_out_cursor++] = getModel().getElement(m_lo_cursor++);

          return;
        }

        m_merge[m_out_cursor++] = getModel().getElement(m_hi_cursor++);
      ...
      case A_COPY:
        ...
        getModel().exchangeElement(m_out_cursor, m_merge[m_out_cursor]);
