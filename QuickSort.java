...
public abstract class QuickSort extends Sort {
  ...
  public void nextStep() {
    ...
    */
    ...
    if (m_lo <= m_hi && getModel().compareElements(m_lo, m_pivot) <= SortModel.O_EQ) {
      ++m_lo;
      return;
    }
    ...
    */
    if (m_lo < m_hi) {
      ...
      for (int ix = r.getLo(); ix <= split; ++ix)
        assert getModel().getElement(ix) <= pivot : ("Order violation at " + ix);
      ...
      m_to_do.pop();
      if (split < r.getHi()) m_to_do.push(new Range(split + 1, r.getHi()));
  ...
  protected abstract int choosePartition(Range r);
  ...
  protected boolean handleBaseCase(Range r) {
    switch (r.getWidth()) {
      ...
      case 1:
        return true;
      case 2:
        if (getModel().compareElements(r.getLo(), r.getHi()) == SortModel.O_GT) {
  ...
  protected class Range {
    ...
    int getWidth() {
      if (m_hi >= m_lo) return (m_hi + 1 - m_lo);
      else return 0;
    }
